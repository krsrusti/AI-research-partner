...
var require_Immutable = /* @__PURE__ */ __commonJSMin(((exports) => {
	...
	*/
	...
	var LAZY = "…";
	var printImmutableEntries = (val, config, indentation, depth, refs, printer, type) => ++depth > config.maxDepth ? printAsLeaf(getImmutableName(type)) : getImmutableName(type) + " {" + (0, _collections.printIteratorEntries)(val.entries(), config, indentation, depth, refs, printer) + "}";
	...
	var printImmutableValues = (val, config, indentation, depth, refs, printer, type) => ++depth > config.maxDepth ? printAsLeaf(getImmutableName(type)) : getImmutableName(type) + " [" + (0, _collections.printIteratorValues)(val.values(), config, indentation, depth, refs, printer) + "]";
	var serialize = (val, config, indentation, depth, refs, printer) => {
...
//#endregion
//#region node_modules/dom-accessibility-api/dist/polyfills/array.from.mjs
...
var SetLike_default = typeof Set === "undefined" ? Set : /* @__PURE__ */ function() {
	function SetLike() {
...
function safeWindow(node) {
	var defaultView = (node.ownerDocument === null ? node : node.ownerDocument).defaultView;
...
function hasAnyConcreteRoles(node, roles) {
	if (isElement(node)) return roles.indexOf(getRole(node)) !== -1;
	return false;
}
//#endregion
...
function hasAbstractRole(node, role) {
	if (!isElement(node)) return false;
	switch (role) {
		case "range": return hasAnyConcreteRoles(node, [
			"meter",
			"progressbar",
			"scrollbar",
			"slider",
		...
		default: throw new TypeError("No knowledge about abstract role '".concat(role, "'. This is likely a bug :("));
	}
}
/**
...
function getSlotContents(slot) {
	...
	if (assignedNodes.length === 0) return arrayFrom(slot.childNodes);
...
function computeTextAlternative(root) {
	...
	function computeMiscTextAlternative(node, context) {
		...
		if (isElement(node) && computedStyleSupportsPseudoElements) {
			var beforeContent = getTextualContent(getComputedStyle(node, "::before"));
			accumulatedText = "".concat(beforeContent, " ").concat(accumulatedText);
		}
		(isHTMLSlotElement(node) ? getSlotContents(node) : arrayFrom(node.childNodes).concat(queryIdRefs(node, "aria-owns"))).forEach(function(child) {
			var result = computeTextAlternative(child, {
				isEmbeddedInLabel: context.isEmbeddedInLabel,
				isReferenced: false,
				recursion: true
			});
			var separator = (isElement(child) ? getComputedStyle(child).getPropertyValue("display") : "inline") !== "inline" ? " " : "";
			accumulatedText += "".concat(separator).concat(result).concat(separator);
		});
		if (isElement(node) && computedStyleSupportsPseudoElements) {
			var afterContent = getTextualContent(getComputedStyle(node, "::after"));
	...
	function computeTextAlternative(current, context) {
		...
		if (skipToStep2E || context.isEmbeddedInLabel || context.isReferenced) {
			if (hasAnyConcreteRoles(current, ["combobox", "listbox"])) {
				...
				if (selectedOptions.length === 0) return isHTMLInputElement(current) ? current.value : "";
...
//#endregion
//#region node_modules/dequal/lite/index.js
var require_lite = /* @__PURE__ */ __commonJSMin(((exports) => {
	...
	for (var i = 0; i < keys.length; i++) {
		...
		if (role) {
			...
			for (var k = 0; k < concepts.length; k++) {
				...
				if (relation.module === "HTML") (function() {
...
//#endregion
//#region node_modules/@testing-library/dom/dist/@testing-library/dom.esm.js
...
var printChildren = (children, config, indentation, depth, refs, printer) => children.map((child) => {
	...
	if (printedChild === "" && typeof child === "object" && child !== null && child.nodeType !== NodeTypeTextNode) return "";
...
function getCodeFrame(frame) {
	...
	const codeFrame = codeFrameColumns(rawFileContents, { start: {
		line,
		column
	} }, {
		highlightCode: true,
...
function getUserCodeFrame() {
	...
	return getCodeFrame((/* @__PURE__ */ new Error()).stack.split("\n").slice(1).find((frame) => !frame.includes("node_modules/")));
}
var TEXT_NODE = 3;
function jestFakeTimersAreEnabled() {
	/* istanbul ignore else */
	if (typeof jest !== "undefined" && jest !== null) return setTimeout._isMockFunction === true || Object.prototype.hasOwnProperty.call(setTimeout, "clock");
	// istanbul ignore next
	return false;
}
function getDocument() {
	/* istanbul ignore if */
	if (typeof window === "undefined") throw new Error("Could not find default container");
	return window.document;
}
function getWindowFromNode(node) {
	if (node.defaultView) return node.defaultView;
	else if (node.ownerDocument && node.ownerDocument.defaultView) return node.ownerDocument.defaultView;
	else if (node.window) return node.window;
...
var shouldHighlight = () => {
	...
	if (typeof colors === "boolean") return colors;
	else return process.versions !== void 0 && process.versions.node !== void 0;
};
var { DOMCollection } = import_build.plugins;
var ELEMENT_NODE = 1;
var COMMENT_NODE = 8;
...
function prettyDOM(dom, maxLength, options) {
	...
	if (typeof maxLength !== "number") maxLength = typeof process !== "undefined" && typeof process.env !== "undefined" && process.env.DEBUG_PRINT_LIMIT || 7e3;
	...
	const debugContent = import_build.format(dom, {
...
function configure(newConfig) {
	if (typeof newConfig === "function") newConfig = newConfig(config);
	config = {
		...config,
		...newConfig
	};
...
function getLabels(container, element, _temp) {
	...
	return labelsId.length ? labelsId.map((labelId) => {
		...
		return {
			...
			formControl: Array.from(label.querySelectorAll("button, input, meter, output, progress, select, textarea")).filter((formControlElement) => formControlElement.matches(selector))[0]
		};
	});
}
function assertNotNullOrUndefined(matcher) {
	if (matcher === null || matcher === void 0) throw new Error("It looks like " + matcher + " was passed instead of a matcher. Did you do something like getByText(" + matcher + ")?");
}
...
function makeNormalizer(_ref) {
	...
	if (typeof trim !== "undefined" || typeof collapseWhitespace !== "undefined") throw new Error("trim and collapseWhitespace are not supported with a normalizer. If you want to use the default trim and collapseWhitespace logic in your normalizer, use \"getDefaultNormalizer({trim, collapseWhitespace})\" and compose that into your normalizer");
	return normalizer;
}
...
var elementRoleList = buildElementRoleList(import_lib.elementRoles);
/**
...
function isSubtreeInaccessible(element) {
	...
	if (element.ownerDocument.defaultView.getComputedStyle(element).display === "none") return true;
	return false;
}
/**
...
function getImplicitAriaRoles(currentNode) {
	for (const { match, roles } of elementRoleList) if (match(currentNode)) return [...roles];
	return [];
}
function buildElementRoleList(elementRolesMap) {
	function makeElementSelector(_ref) {
		...
		return "" + name + attributes.map((_ref2) => {
			...
			if (typeof value !== "undefined") return "[" + attributeName + "=\"" + value + "\"]";
			else if (shouldNotExist) return ":not([" + attributeName + "])";
			else if (shouldBeNonEmpty) return "[" + attributeName + "]:not([" + attributeName + "=\"\"])";
	...
	function match(element) {
		...
		if (typeTextIndex >= 0) attributes = [...attributes.slice(0, typeTextIndex), ...attributes.slice(typeTextIndex + 1)];
	...
	let result = [];
	for (const [element, roles] of elementRolesMap.entries()) result = [...result, {
		match: match(element),
...
function prettyRoles(dom, _ref6) {
	...
	return Object.entries(roles).filter((_ref7) => {
		...
		const elementsString = elements.map((el) => {
			const nameString = "Name \"" + computeAccessibleName(el, { computedStyleSupportsPseudoElements: getConfig().computedStyleSupportsPseudoElements }) + "\":\n";
			const domString = prettyDOM(el.cloneNode(false));
			if (includeDescription) {
				const descriptionString = "Description \"" + computeAccessibleDescription(el, { computedStyleSupportsPseudoElements: getConfig().computedStyleSupportsPseudoElements }) + "\":\n";
...
function computeHeadingLevel(element) {
	return element.getAttribute("aria-level") && Number(element.getAttribute("aria-level")) || {
		H1: 1,
		H2: 2,
		H3: 3,
		H4: 4,
...
function getSuggestedQuery(element, variant, method) {
	...
	if (role !== "generic" && canSuggest("Role", method, role)) return makeSuggestion("Role", element, role, {
		variant,
		name: computeAccessibleName(element, { computedStyleSupportsPseudoElements: getConfig().computedStyleSupportsPseudoElements })
	});
	...
	if (canSuggest("DisplayValue", method, element.value)) return makeSuggestion("DisplayValue", element, normalize(element.value), { variant });
...
function waitFor(callback, _ref) {
	let { container = getDocument(), timeout = getConfig().asyncUtilTimeout, showOriginalStackTrace = getConfig().showOriginalStackTrace, stackTraceError, interval = 50, onTimeout = (error) => {
		Object.defineProperty(error, "message", { value: getConfig().getElementError(error.message, container).message });
		return error;
	...
	return new Promise(async (resolve, reject) => {
		...
		if (usingJestFakeTimers) {
			...
			while (!finished) {
				if (!jestFakeTimersAreEnabled()) {
					const error = /* @__PURE__ */ new Error("Changed from using fake timers to real timers while using waitFor. This is not allowed and will result in very strange behavior. Please ensure you're awaiting all async things your test is doing before changing to real timers. For more info, please go to https://github.com/testing-library/dom-testing-library/issues/830");
		...
		function checkRealTimersCallback() {
			if (jestFakeTimersAreEnabled()) {
				const error = /* @__PURE__ */ new Error("Changed from using real timers to fake timers while using waitFor. This is not allowed and will result in very strange behavior. Please ensure you're awaiting all async things your test is doing before changing to fake timers. For more info, please go to https://github.com/testing-library/dom-testing-library/issues/830");
				if (!showOriginalStackTrace) copyStackTrace(error, stackTraceError);
				return reject(error);
		...
		function handleTimeout() {
			...
			if (lastError) {
				...
				error = /* @__PURE__ */ new Error("Timed out in waitFor.");
				if (!showOriginalStackTrace) copyStackTrace(error, stackTraceError);
			}
...
function waitForWrapper(callback, options) {
	const stackTraceError = /* @__PURE__ */ new Error("STACK_TRACE_MESSAGE");
...
function buildQueries(queryAllBy, getMultipleError, getMissingError) {
	...
	return [
		queryBy,
		wrapAllByQueryWithSuggestion(getAllBy, queryAllBy.name.replace("query", "get"), "getAll"),
		getByWithSuggestions,
		makeFindQuery(wrapAllByQueryWithSuggestion(getAllBy, queryAllBy.name, "findAll")),
		makeFindQuery(wrapSingleQueryWithSuggestion(getBy, queryAllBy.name, "find"))
	];
}
...
var getMissingError$5 = function(c, text, options) {
	...
	const normalizedText = makeNormalizer({
		collapseWhitespace,
		trim,
		normalizer
	})(text.toString());
...
var queryAllByDisplayValue = function(container, value, _temp) {
	...
	return Array.from(container.querySelectorAll("input,textarea,select")).filter((node) => {
		if (node.tagName === "SELECT") return Array.from(node.options).filter((option) => option.selected).some((optionNode) => matcher(getNodeText(optionNode), optionNode, value, matchNormalizer));
...
var queryAllByRole = function(container, role, _temp) {
	...
	if (selected !== void 0) {
		var _allRoles$get;
		if (((_allRoles$get = import_lib.roles.get(role)) == null ? void 0 : _allRoles$get.props["aria-selected"]) === void 0) throw new Error("\"aria-selected\" is not supported on role \"" + role + "\".");
	}
	if (busy !== void 0) {
		var _allRoles$get2;
		if (((_allRoles$get2 = import_lib.roles.get(role)) == null ? void 0 : _allRoles$get2.props["aria-busy"]) === void 0) throw new Error("\"aria-busy\" is not supported on role \"" + role + "\".");
	}
	if (checked !== void 0) {
		var _allRoles$get3;
		if (((_allRoles$get3 = import_lib.roles.get(role)) == null ? void 0 : _allRoles$get3.props["aria-checked"]) === void 0) throw new Error("\"aria-checked\" is not supported on role \"" + role + "\".");
	}
	if (pressed !== void 0) {
		var _allRoles$get4;
		if (((_allRoles$get4 = import_lib.roles.get(role)) == null ? void 0 : _allRoles$get4.props["aria-pressed"]) === void 0) throw new Error("\"aria-pressed\" is not supported on role \"" + role + "\".");
	}
	if (current !== void 0) {
		var _allRoles$get5;
		/* istanbul ignore next */
		if (((_allRoles$get5 = import_lib.roles.get(role)) == null ? void 0 : _allRoles$get5.props["aria-current"]) === void 0) throw new Error("\"aria-current\" is not supported on role \"" + role + "\".");
	}
	if (level !== void 0) {
	...
	if (valueNow !== void 0) {
		var _allRoles$get6;
		if (((_allRoles$get6 = import_lib.roles.get(role)) == null ? void 0 : _allRoles$get6.props["aria-valuenow"]) === void 0) throw new Error("\"aria-valuenow\" is not supported on role \"" + role + "\".");
	}
	if (valueMax !== void 0) {
		var _allRoles$get7;
		if (((_allRoles$get7 = import_lib.roles.get(role)) == null ? void 0 : _allRoles$get7.props["aria-valuemax"]) === void 0) throw new Error("\"aria-valuemax\" is not supported on role \"" + role + "\".");
	}
	if (valueMin !== void 0) {
		var _allRoles$get8;
		if (((_allRoles$get8 = import_lib.roles.get(role)) == null ? void 0 : _allRoles$get8.props["aria-valuemin"]) === void 0) throw new Error("\"aria-valuemin\" is not supported on role \"" + role + "\".");
	}
	if (valueText !== void 0) {
		var _allRoles$get9;
		if (((_allRoles$get9 = import_lib.roles.get(role)) == null ? void 0 : _allRoles$get9.props["aria-valuetext"]) === void 0) throw new Error("\"aria-valuetext\" is not supported on role \"" + role + "\".");
	}
	if (expanded !== void 0) {
		var _allRoles$get0;
		if (((_allRoles$get0 = import_lib.roles.get(role)) == null ? void 0 : _allRoles$get0.props["aria-expanded"]) === void 0) throw new Error("\"aria-expanded\" is not supported on role \"" + role + "\".");
	}
	...
	return Array.from(container.querySelectorAll(makeRoleSelector(role))).filter((node) => {
		...
		if (valueNow !== void 0 || valueMax !== void 0 || valueMin !== void 0 || valueText !== void 0) {
			...
			if (valueNow !== void 0) valueMatches && (valueMatches = valueNow === computeAriaValueNow(element));
			if (valueMax !== void 0) valueMatches && (valueMatches = valueMax === computeAriaValueMax(element));
			if (valueMin !== void 0) valueMatches && (valueMatches = valueMin === computeAriaValueMin(element));
			if (valueText !== void 0) {
		...
		if (name === void 0) return true;
		return matches(computeAccessibleName(element, { computedStyleSupportsPseudoElements: getConfig().computedStyleSupportsPseudoElements }), element, name, (text) => text);
	}).filter((element) => {
		if (description === void 0) return true;
		return matches(computeAccessibleDescription(element, { computedStyleSupportsPseudoElements: getConfig().computedStyleSupportsPseudoElements }), element, description, (text) => text);
	}).filter((element) => {
		return hidden === false ? isInaccessible(element, { isSubtreeInaccessible: cachedIsSubtreeInaccessible }) === false : true;
	});
};
function makeRoleSelector(role) {
	var _roleElements$get;
	const explicitRoleSelector = "*[role~=\"" + role + "\"]";
	const roleRelations = (_roleElements$get = import_lib.roleElements.get(role)) != null ? _roleElements$get : /* @__PURE__ */ new Set();
...
var getMissingError$1 = function(container, role, _temp3) {
	...
	let roleMessage;
	if (roles.length === 0) if (hidden === false) roleMessage = "There are no accessible roles. But there might be some inaccessible roles. If you wish to access them, then set the `hidden` option to `true`. Learn more about this here: https://testing-library.com/docs/dom-testing-library/api-queries#byrole";
	...
	let descriptionHint = "";
	if (description === void 0) descriptionHint = "";
...
function getQueriesForElement(element, queries$1, initialValue) {
	...
	return Object.keys(queries$1).reduce((helpers, key) => {
		helpers[key] = queries$1[key].bind(null, element);
		return helpers;
	}, initialValue);
}
...
async function waitForElementToBeRemoved(callback, options) {
	const timeoutError = /* @__PURE__ */ new Error("Timed out in waitForElementToBeRemoved.");
	if (typeof callback !== "function") {
		initialCheck(callback);
		const getRemainingElements = (Array.isArray(callback) ? callback : [callback]).map((element) => {
...
function createEvent(eventName, node, init, _temp) {
	...
	if (value !== void 0) setNativeValue(node, value);
	if (files !== void 0) Object.defineProperty(node, "files", {
		configurable: true,
		enumerable: true,
		writable: true,
		value: files
 ...
 /* istanbul ignore if  */
		if (typeof window.DataTransfer === "function") Object.defineProperty(event, dataTransferKey, { value: Object.getOwnPropertyNames(dataTransferValue).reduce((acc, propName) => {
			Object.defineProperty(acc, propName, { value: dataTransferValue[propName] });
			return acc;
		}, new window.DataTransfer()) });
		else Object.defineProperty(event, dataTransferKey, { value: dataTransferValue });
	});
	return event;
}
...
//#endregion
export { queries as $, getByDisplayValue as A, getMultipleElementsFoundError as B, getAllByLabelTextWithSuggestions as C, wrapSingleQueryWithSuggestion as Ct, getAllByText as D, getAllByTestId as E, getByText as F, import_build as G, getQueriesForElement as H, getByTitle as I, logRoles as J, isInaccessible as K, getConfig as L, getByPlaceholderText as M, getByRole as N, getAllByTitle as O, getByTestId as P, prettyDOM as Q, getDefaultNormalizer as R, getAllByDisplayValue as S, wrapAllByQueryWithSuggestion as St, getAllByRole as T, getRoles as U, getNodeText as V, getSuggestedQuery as W, makeGetAllQuery as X, makeFindQuery as Y, makeSingleQuery as Z, findByTestId as _, queryByTitle as _t, findAllByDisplayValue as a, queryAllByRoleWithSuggestions as at, fireEvent as b, waitForElementToBeRemoved as bt, findAllByRole as c, queryAllByTitleWithSuggestions as ct, findAllByTitle as d, queryByDisplayValue as dt, queryAllByAltTextWithSuggestions as et, findByAltText as f, queryByLabelText as ft, findByRole as g, queryByText as gt, findByPlaceholderText as h, queryByTestId as ht, findAllByAltText as i, queryAllByPlaceholderTextWithSuggestions as it, getByLabelTextWithSuggestions as j, getByAltText as k, findAllByTestId as l, queryByAltText as lt, findByLabelText as m, queryByRole as mt, configure as n, queryAllByDisplayValueWithSuggestions as nt, findAllByLabelText as o, queryAllByTestIdWithSuggestions as ot, findByDisplayValue as p, queryByPlaceholderText as pt, logDOM as q, createEvent as r, queryAllByLabelTextWithSuggestions as rt, findAllByPlaceholderText as s, queryAllByTextWithSuggestions as st, buildQueries as t, queryAllByAttribute as tt, findAllByText as u, queryByAttribute as ut, findByText as v, queryHelpers as vt, getAllByPlaceholderText as w, getAllByAltText as x, waitForWrapper as xt, findByTitle as y, screen as yt, getElementError as z };
