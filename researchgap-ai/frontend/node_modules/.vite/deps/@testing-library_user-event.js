...
import { L as getConfig } from "./dom.esm-DQXUxhYP.js";
//#region node_modules/@testing-library/user-event/dist/esm/utils/misc/isElementType.js
...
//#endregion
//#region node_modules/@testing-library/user-event/dist/esm/utils/misc/getWindow.js
function getWindow(node) {
	var _node_ownerDocument;
	if (isDocument$1(node) && node.defaultView) return node.defaultView;
	else if ((_node_ownerDocument = node.ownerDocument) === null || _node_ownerDocument === void 0 ? void 0 : _node_ownerDocument.defaultView) return node.ownerDocument.defaultView;
...
//#endregion
//#region node_modules/@testing-library/user-event/dist/esm/utils/dataTransfer/Blob.js
...
//#endregion
//#region node_modules/@testing-library/user-event/dist/esm/utils/dataTransfer/FileList.js
function createFileList(window, files) {
	...
	/* istanbul ignore else */ if (window.FileList) Object.setPrototypeOf(list, window.FileList.prototype);
	Object.freeze(list);
	return list;
}
//#endregion
//#region node_modules/@testing-library/user-event/dist/esm/utils/dataTransfer/DataTransfer.js
...
var DataTransferItemStub = class {
	...
	getAsString(callback) {
		if (typeof this.data === "string") callback(this.data);
	}
...
function createDataTransferStub(window) {
	return new class DataTransferStub {
		getData(format) {
			...
			let text = "";
			match === null || match === void 0 || match.getAsString((t) => {
				text = t;
			});
			return text;
		}
		setData(format, data) {
			...
			if (matchIndex >= 0) this.items.splice(matchIndex, 1, item);
			else this.items.push(item);
		}
		clearData(format) {
			if (format) {
				const matchIndex = this.items.findIndex(getTypeMatcher(format, true));
				if (matchIndex >= 0) this.items.remove(matchIndex);
			} else this.items.clear();
		}
		get types() {
			const t = [];
			if (this.files.length) t.push("Files");
...
async function getBlobFromDataTransferItem(window, item) {
	...
	return new window.Blob([await new Promise((r) => item.getAsString(r))], { type: item.type });
}
//#endregion
//#region node_modules/@testing-library/user-event/dist/esm/utils/dataTransfer/Clipboard.js
...
function createClipboardItem(window, ...blobs) {
	...
	/* istanbul ignore if */ if (typeof window.ClipboardItem !== "undefined") return new window.ClipboardItem(dataMap);
	return new class ClipboardItem {
		...
		async getType(type) {
			...
			if (!value) throw new Error(`${type} is not one of the available MIME types on this item.`);
	...
	return Object.assign(new class Clipboard extends window.EventTarget {
		...
		async readText() {
			...
			for (const item of this.items) {
				...
				if (type) text += await item.getType(type).then((b) => readBlobText(b, window.FileReader));
			}
			return text;
		}
		async write(data) {
...
function attachClipboardStubToView(window) {
	if (isClipboardStub(window.navigator.clipboard)) return window.navigator.clipboard[ClipboardStubControl];
	...
	const control = {
		...
		detachClipboardStub: () => {
			/* istanbul ignore if */ if (realClipboard) Object.defineProperty(window.navigator, "clipboard", realClipboard);
			else Object.defineProperty(window.navigator, "clipboard", {
				value: void 0,
				configurable: true
			});
		}
...
function resetClipboardStubOnView(window) {
	if (isClipboardStub(window.navigator.clipboard)) window.navigator.clipboard[ClipboardStubControl].resetClipboardStub();
}
function detachClipboardStubFromView(window) {
	if (isClipboardStub(window.navigator.clipboard)) window.navigator.clipboard[ClipboardStubControl].detachClipboardStub();
}
async function readDataTransferFromClipboard(document) {
	...
	for (const item of items) for (const type of item.types) dt.setData(type, await item.getType(type).then((b) => readBlobText(b, window.FileReader)));
	return dt;
}
...
var g = globalThis;
/* istanbul ignore else */ if (typeof g.afterEach === "function") g.afterEach(() => resetClipboardStubOnView(globalThis.window));
/* istanbul ignore else */ if (typeof g.afterAll === "function") g.afterAll(() => detachClipboardStubFromView(globalThis.window));
//#endregion
//#region node_modules/@testing-library/user-event/dist/esm/utils/focus/selector.js
...
//#endregion
//#region node_modules/@testing-library/user-event/dist/esm/utils/focus/isFocusable.js
function isFocusable(element) {
	return element.matches(FOCUSABLE_SELECTOR);
}
//#endregion
//#region node_modules/@testing-library/user-event/dist/esm/utils/misc/cloneEvent.js
function cloneEvent(event) {
	return new event.constructor(event.type, event);
}
//#endregion
//#region node_modules/@testing-library/user-event/dist/esm/utils/misc/isDisabled.js
function isDisabled(element) {
	for (let el = element; el; el = el.parentElement) if (isElementType(el, [
		"button",
		"input",
		"select",
		"textarea",
		...
		if (el.hasAttribute("disabled") && !((_el_querySelector = el.querySelector(":scope > legend")) === null || _el_querySelector === void 0 ? void 0 : _el_querySelector.contains(element))) return true;
	} else if (el.tagName.includes("-")) {
		if (el.constructor.formAssociated && el.hasAttribute("disabled")) return true;
	}
	return false;
}
//#endregion
//#region node_modules/@testing-library/user-event/dist/esm/utils/focus/getActiveElement.js
function getActiveElement(document) {
	const activeElement = document.activeElement;
	if (activeElement === null || activeElement === void 0 ? void 0 : activeElement.shadowRoot) return getActiveElement(activeElement.shadowRoot);
	else {
		if (isDisabled(activeElement)) return document.ownerDocument ? document.ownerDocument.body : document.body;
...
//#endregion
//#region node_modules/@testing-library/user-event/dist/esm/utils/misc/findClosest.js
...
//#endregion
//#region node_modules/@testing-library/user-event/dist/esm/utils/edit/isContentEditable.js
...
//#endregion
//#region node_modules/@testing-library/user-event/dist/esm/utils/click/isClickableInput.js
...
//#endregion
//#region node_modules/@testing-library/user-event/dist/esm/utils/edit/isEditable.js
...
//#endregion
//#region node_modules/@testing-library/user-event/dist/esm/utils/focus/selection.js
/**
...
//#endregion
//#region node_modules/@testing-library/user-event/dist/esm/event/selection/updateSelectionOnFocus.js
/**
* Reset the Document Selection when moving focus into an element
* with own selection implementation.
*/ function updateSelectionOnFocus(element) {
	const selection = element.ownerDocument.getSelection();
	/* istanbul ignore if */ if (!(selection === null || selection === void 0 ? void 0 : selection.focusNode)) return;
...
//#endregion
//#region node_modules/@testing-library/user-event/dist/esm/event/wrapEvent.js
function wrapEvent(cb, _element) {
	return getConfig().eventWrapper(cb);
}
//#endregion
//#region node_modules/@testing-library/user-event/dist/esm/event/focus.js
/**
...
*/ function focusElement(element) {
	...
	if ((target !== null && target !== void 0 ? target : element.ownerDocument.body) === activeElement) return;
	else if (target) wrapEvent(() => target.focus());
	else wrapEvent(() => activeElement === null || activeElement === void 0 ? void 0 : activeElement.blur());
	updateSelectionOnFocus(target !== null && target !== void 0 ? target : element.ownerDocument.body);
}
function blurElement(element) {
	if (!isFocusable(element)) return;
	if (!(getActiveElement(element.ownerDocument) === element)) return;
	wrapEvent(() => element.blur());
}
//#endregion
//#region node_modules/@testing-library/user-event/dist/esm/event/behavior/registry.js
var behavior = {};
//#endregion
//#region node_modules/@testing-library/user-event/dist/esm/event/behavior/click.js
behavior.click = (event, target, instance) => {
	...
	else if (isElementType(target, "input", { type: "file" })) return () => {
		blurElement(target);
...
//#endregion
//#region node_modules/@testing-library/user-event/dist/esm/document/UI.js
...
function setUIValue(element, value) {
	if (element[InitialValue] === void 0) element[InitialValue] = element.value;
	element[UIValue] = value;
	element.value = Object.assign(new String(value), { [UIValue]: true });
}
...
function setUISelection(element, { focusOffset: focusOffsetParam, anchorOffset: anchorOffsetParam = focusOffsetParam }, mode = "replace") {
	...
	if (element.selectionStart === startOffset && element.selectionEnd === endOffset) return;
	const startObj = Object.assign(new Number(startOffset), { [UISelection]: true });
	try {
...
//#endregion
//#region node_modules/@testing-library/user-event/dist/esm/utils/edit/timeValue.js
...
function build(onlyDigitsValue, index) {
	...
	const parsedMinutes = parseInt(onlyDigitsValue.slice(index), 10);
...
//#endregion
//#region node_modules/@testing-library/user-event/dist/esm/utils/edit/maxLength.js
...
//#endregion
//#region node_modules/@testing-library/user-event/dist/esm/utils/focus/cursor.js
function getNextCursorPosition(node, offset, direction, inputType) {
	if (isTextNode(node) && offset + direction >= 0 && offset + direction <= node.nodeValue.length) return {
		node,
		offset: offset + direction
	};
	const nextNode = getNextCharacterContentNode(node, offset, direction);
	if (nextNode) if (isTextNode(nextNode)) return {
		node: nextNode,
		...
		if (!nextPlusOne) {
			if (direction < 0 && inputType === "deleteContentBackward") return {
				node: nextNode.parentNode,
				offset: getOffset(nextNode)
			};
			return;
		} else if (isTextNode(nextPlusOne)) return {
			node: nextPlusOne,
			offset: direction > 0 ? 0 : nextPlusOne.nodeValue.length
		};
		else if (direction < 0 && isElementType(nextPlusOne, "br")) return {
			node: nextNode.parentNode,
			offset: getOffset(nextNode)
		};
		else return {
...
function getNextCharacterContentNode(node, offset, direction) {
	...
	if (offset !== void 0 && isElement(node) && nextOffset >= 0 && nextOffset < node.children.length) node = node.children[nextOffset];
...
function isTreatedAsCharacterContent(node) {
	...
	if (isElement(node)) {
		...
		else if (isElementType(node, "br")) return true;
	}
	return false;
}
...
function walkNodes(node, direction, callback) {
	for (;;) {
		...
		} else if (node.parentNode && (!isElement(node.parentNode) || !isContentEditable(node.parentNode) && node.parentNode !== ((_node_ownerDocument = node.ownerDocument) === null || _node_ownerDocument === void 0 ? void 0 : _node_ownerDocument.body))) node = node.parentNode;
		else break;
	}
}
function getDescendant(node, direction) {
	while (node.hasChildNodes()) node = node[`${direction}Child`];
	return node;
}
//#endregion
//#region node_modules/@testing-library/user-event/dist/esm/document/trackValue.js
...
function startTrackValue(element) {
	if (!isReact17Element(element)) return;
...
function trackOrSetValue(element, v) {
	...
	(_element_TrackChanges = element[TrackChanges]) === null || _element_TrackChanges === void 0 || (_element_TrackChanges_tracked = _element_TrackChanges.tracked) == null || _element_TrackChanges_tracked.push(v);
...
function commitValueAfterInput(element, cursorOffset) {
	...
	if (!(changes === null || changes === void 0 ? void 0 : (_changes_tracked = changes.tracked) === null || _changes_tracked === void 0 ? void 0 : _changes_tracked.length)) return;
	...
	if (!isJustReactStateUpdate) setUIValueClean(element);
	if (hasUISelection(element)) setUISelection(element, { focusOffset: isJustReactStateUpdate ? cursorOffset : element.value.length });
}
//#endregion
//#region node_modules/@testing-library/user-event/dist/esm/event/selection/getTargetTypeAndSelection.js
/**
* Determine which selection logic and selection ranges to consider.
*/ function getTargetTypeAndSelection(node) {
	const element = getElement(node);
	if (element && hasOwnSelection(element)) return {
		type: "input",
		selection: getUISelection(element)
	};
	const selection = element === null || element === void 0 ? void 0 : element.ownerDocument.getSelection();
	return {
		type: getContentEditable(node) && (selection === null || selection === void 0 ? void 0 : selection.anchorNode) && getContentEditable(selection.anchorNode) ? "contenteditable" : "default",
		selection
	};
}
...
//#endregion
//#region node_modules/@testing-library/user-event/dist/esm/event/selection/getInputRange.js
/**
...
//#endregion
//#region node_modules/@testing-library/user-event/dist/esm/event/selection/setSelection.js
/**
* Set the selection
*/ function setSelection({ focusNode, focusOffset, anchorNode = focusNode, anchorOffset = focusOffset }) {
	var _anchorNode_ownerDocument_getSelection, _anchorNode_ownerDocument;
	if (getTargetTypeAndSelection(focusNode).type === "input") return setUISelection(focusNode, {
		anchorOffset,
		focusOffset
	});
	(_anchorNode_ownerDocument = anchorNode.ownerDocument) === null || _anchorNode_ownerDocument === void 0 || (_anchorNode_ownerDocument_getSelection = _anchorNode_ownerDocument.getSelection()) == null || _anchorNode_ownerDocument_getSelection.setBaseAndExtent(anchorNode, anchorOffset, focusNode, focusOffset);
}
//#endregion
//#region node_modules/@testing-library/user-event/dist/esm/event/input.js
...
function input(instance, element, data, inputType = "insertText") {
	...
	/* istanbul ignore if */ if (!inputRange) return;
	if (!isDateOrTime(element)) {
		if (!instance.dispatchUIEvent(element, "beforeinput", {
			inputType,
			data
		})) return;
	}
	if ("startContainer" in inputRange) editContenteditable(instance, element, inputRange, data, inputType);
	else editInputElement(instance, element, inputRange, data, inputType);
}
function editContenteditable(instance, element, inputRange, data, inputType) {
	...
	if (!inputRange.collapsed) {
		...
		if (nextPosition) {
			...
			if (delRange.comparePoint(nextPosition.node, nextPosition.offset) < 0) delRange.setStart(nextPosition.node, nextPosition.offset);
			else delRange.setEnd(nextPosition.node, nextPosition.offset);
	...
	if (del || data) instance.dispatchUIEvent(element, "input", { inputType });
}
function editInputElement(instance, element, inputRange, data, inputType) {
	...
	if (newValue === oldValue && newOffset === inputRange.startOffset && newOffset === inputRange.endOffset) return;
	if (isElementType(element, "input", { type: "number" }) && !isValidNumberInput(newValue)) return;
	...
	} else commitInput(instance, element, newOffset, {
		data,
		inputType
	});
}
function calculateNewValue(inputData, node, { startOffset, endOffset }, inputType) {
	...
	let newValue = `${prolog}${inputData}${value.substring(epilogStart, value.length)}`;
...
//#endregion
//#region node_modules/@testing-library/user-event/dist/esm/event/behavior/cut.js
behavior.cut = (event, target, instance) => {
	return () => {
		if (isEditable(target)) input(instance, target, "", "deleteByCut");
	};
};
//#endregion
//#region node_modules/@testing-library/user-event/dist/esm/document/getValueOrTextContent.js
...
//#endregion
//#region node_modules/@testing-library/user-event/dist/esm/utils/misc/isVisible.js
...
//#endregion
//#region node_modules/@testing-library/user-event/dist/esm/utils/focus/getTabDestination.js
function getTabDestination(activeElement, shift) {
	...
	if (Number(activeElement.getAttribute("tabindex")) >= 0) enabledElements.sort((a, b) => {
	...
	const activeRadioGroup = isElementType(activeElement, "input", { type: "radio" }) ? activeElement.name : void 0;
	enabledElements.forEach((currentElement) => {
		...
		if (isElementType(el, "input", { type: "radio" }) && el.name) {
			...
			if (typeof checkedRadio[el.name] !== "undefined") return;
		}
		prunedElements.push(el);
	});
	for (let index = prunedElements.findIndex((el) => el === activeElement);;) {
		index += shift ? -1 : 1;
		if (index === prunedElements.length) index = 0;
		else if (index === -1) index = prunedElements.length - 1;
		if (prunedElements[index] === activeElement || prunedElements[index] === document.body || isVisible(prunedElements[index])) return prunedElements[index];
	}
}
//#endregion
//#region node_modules/@testing-library/user-event/dist/esm/event/selection/moveSelection.js
/**
* Move the selection
*/ function moveSelection(node, direction) {
	if (hasOwnSelection(node)) {
		...
		if (!(selection === null || selection === void 0 ? void 0 : selection.focusNode)) return;
		if (selection.isCollapsed) {
			const nextPosition = getNextCursorPosition(selection.focusNode, selection.focusOffset, direction);
			if (nextPosition) setSelection({
...
//#endregion
//#region node_modules/@testing-library/user-event/dist/esm/event/selection/selectAll.js
/**
* Expand a selection like the browser does when pressing Ctrl+A.
*/ function selectAll(target) {
	if (hasOwnSelection(target)) return setSelection({
		focusNode: target,
		anchorOffset: 0,
...
//#endregion
//#region node_modules/@testing-library/user-event/dist/esm/event/selection/setSelectionRange.js
/**
...
*/ function setSelectionRange(element, anchorOffset, focusOffset) {
	...
	if (hasOwnSelection(element)) return setSelection({
		focusNode: element,
		anchorOffset,
		focusOffset
	});
	/* istanbul ignore else */ if (isContentEditable(element) && ((_element_firstChild = element.firstChild) === null || _element_firstChild === void 0 ? void 0 : _element_firstChild.nodeType) === 3) return setSelection({
...
//#endregion
//#region node_modules/@testing-library/user-event/dist/esm/event/radio.js
function walkRadio(instance, el, direction) {
	...
	for (let i = group.findIndex((e) => e === el) + direction;; i += direction) {
		if (!group[i]) i = direction > 0 ? 0 : group.length - 1;
		if (group[i] === el) return;
		if (isDisabled(group[i])) continue;
...
//#endregion
//#region node_modules/@testing-library/user-event/dist/esm/event/behavior/keydown.js
...
var keydownBehavior = {
	ArrowDown: (event, target, instance) => {
		/* istanbul ignore else */ if (isElementType(target, "input", { type: "radio" })) return () => walkRadio(instance, target, 1);
	},
	...
	Tab: (event, target, instance) => {
		return () => {
			...
			focusElement(dest);
			if (hasOwnSelection(dest)) setUISelection(dest, {
				anchorOffset: 0,
...
//#endregion
//#region node_modules/@testing-library/user-event/dist/esm/event/behavior/keypress.js
behavior.keypress = (event, target, instance) => {
	if (event.key === "Enter") {
		if (isElementType(target, "button") || isElementType(target, "input") && ClickInputOnEnter.includes(target.type) || isElementType(target, "a") && Boolean(target.href)) return () => {
			...
			else if (form && SubmitSingleInputOnEnter.includes(target.type) && form.querySelectorAll("input").length === 1) return () => instance.dispatchUIEvent(form, "submit");
			else return;
		}
	}
...
//#endregion
//#region node_modules/@testing-library/user-event/dist/esm/event/behavior/keyup.js
...
//#endregion
//#region node_modules/@testing-library/user-event/dist/esm/event/behavior/paste.js
behavior.paste = (event, target, instance) => {
	if (isEditable(target)) return () => {
		...
		if (insertData) input(instance, target, insertData, "insertFromPaste");
	};
};
//#endregion
//#region node_modules/@testing-library/user-event/dist/esm/event/eventMap.js
var eventMap = {
	auxclick: {
...
//#endregion
//#region node_modules/@testing-library/user-event/dist/esm/event/createEvent.js
...
/* istanbul ignore next */ function getEventConstructors(window) {
	...
	return {
		...
		DragEvent,
		PointerEvent,
		TouchEvent: (_window_TouchEvent = window.TouchEvent) !== null && _window_TouchEvent !== void 0 ? _window_TouchEvent : class TouchEvent extends UIEvent {}
	};
}
function assignProps(obj, props) {
	for (const [key, value] of Object.entries(props)) Object.defineProperty(obj, key, { get: () => value !== null && value !== void 0 ? value : null });
}
...
function initFocusEvent(event, { relatedTarget }) {
	assignProps(event, { relatedTarget });
}
...
//#endregion
//#region node_modules/@testing-library/user-event/dist/esm/event/dispatchEvent.js
function dispatchUIEvent(target, type, init, preventDefault = false) {
	if (isMouseEvent(type) || isKeyboardEvent(type)) init = {
		...init,
...
function dispatchEvent(target, event, preventDefault = false) {
	...
	if (behaviorImplementation) {
		...
		Object.defineProperty(event, "defaultPrevented", { get: () => defaultPrevented });
		Object.defineProperty(event, "preventDefault", { value: () => {
			defaultPrevented = event.cancelable;
		} });
		wrapEvent(() => target.dispatchEvent(event));
		if (!defaultPrevented) behaviorImplementation();
...
//#endregion
//#region node_modules/@testing-library/user-event/dist/esm/document/patchFocus.js
var patched = Symbol("patched focus/blur methods");
function patchFocus(HTMLElement) {
	if (HTMLElement.prototype[patched]) return;
	...
	function patchedFocus(options) {
		if (this.ownerDocument.visibilityState !== "hidden") return focus.call(this, options);
		const blurred = getActiveTarget(this.ownerDocument);
		if (blurred === this) return;
		const thisCall = Symbol("focus call");
		activeCall = thisCall;
		if (blurred) {
			blur.call(blurred);
			dispatchDOMEvent(blurred, "blur", { relatedTarget: this });
			dispatchDOMEvent(blurred, "focusout", { relatedTarget: activeCall === thisCall ? this : null });
		}
		if (activeCall === thisCall) {
			focus.call(this, options);
			dispatchDOMEvent(this, "focus", { relatedTarget: blurred });
		}
		if (activeCall === thisCall) dispatchDOMEvent(this, "focusin", { relatedTarget: blurred });
	}
	function patchedBlur() {
		if (this.ownerDocument.visibilityState !== "hidden") return blur.call(this);
		const blurred = getActiveTarget(this.ownerDocument);
		if (blurred !== this) return;
		activeCall = Symbol("blur call");
		blur.call(this);
...
//#endregion
//#region node_modules/@testing-library/user-event/dist/esm/document/interceptor.js
...
function prepareInterceptor(element, propName, interceptorImpl) {
	...
	/* istanbul ignore if */ if (typeof (prototypeDescriptor === null || prototypeDescriptor === void 0 ? void 0 : prototypeDescriptor[target]) !== "function" || prototypeDescriptor[target][Interceptor]) throw new Error(`Element ${element.tagName} does not implement "${String(propName)}".`);
	function intercept(...args) {
		...
		if (target === "set") realFunc.call(this, realArgs);
		else realFunc.call(this, ...realArgs);
		then === null || then === void 0 || then();
	}
...
function sanitizeValue(element, v) {
	if (isElementType(element, "input", { type: "number" }) && String(v) !== "" && !Number.isNaN(Number(v))) return String(Number(v));
	return String(v);
}
...
//#endregion
//#region node_modules/@testing-library/user-event/dist/esm/document/prepareDocument.js
var isPrepared = Symbol("Node prepared with document state workarounds");
function prepareDocument(document) {
	if (document[isPrepared]) return;
	...
	if (document.activeElement) prepareElement(document.activeElement);
	document.addEventListener("blur", (e) => {
		const el = e.target;
		const initialValue = getInitialValue(el);
		if (initialValue !== void 0) {
			if (el.value !== initialValue) dispatchDOMEvent(el, "change");
...
//#endregion
//#region node_modules/@testing-library/user-event/dist/esm/utils/misc/getDocumentFromNode.js
...
//#endregion
//#region node_modules/@testing-library/user-event/dist/esm/utils/misc/level.js
...
//#endregion
//#region node_modules/@testing-library/user-event/dist/esm/utils/misc/wait.js
...
//#endregion
//#region node_modules/@testing-library/user-event/dist/esm/options.js
...
//#endregion
//#region node_modules/@testing-library/user-event/dist/esm/system/keyboard.js
...
var KeyboardHost = class {
	...
	/** Press a key */ async keydown(instance, keyDef) {
		...
		if (isModifierKey(key)) this.modifiers[key] = true;
		...
		if (unprevented) this.pressed.setUnprevented(code);
		if (unprevented && this.hasKeyPress(key)) instance.dispatchUIEvent(getActiveElementOrBody(instance.config.document), "keypress", {
			key,
			code,
	...
	/** Release a key */ async keyup(instance, keyDef) {
		...
		if (isModifierKey(key) && !this.pressed.values().find((p) => p.keyDef.key === key)) this.modifiers[key] = false;
		instance.dispatchUIEvent(getActiveElementOrBody(instance.config.document), "keyup", {
			key,
			code
		}, !unprevented);
		if (isModifierLock(key) && this.modifiers[key]) if (this.modifierLockStart[key]) this.modifierLockStart[key] = false;
		else this.modifiers[key] = false;
	}
	setKeydownTarget(target) {
		if (target !== this.lastKeydownTarget) this.carryChar = "";
	...
	constructor(system) {
		...
		_define_property$6(this, "pressed", new class {
			...
			setUnprevented(code) {
				...
				if (o) o.unpreventedDefault = true;
			}
			isUnprevented(code) {
...
//#endregion
//#region node_modules/@testing-library/user-event/dist/esm/keyboard/keyMap.js
/**
...
//#endregion
//#region node_modules/@testing-library/user-event/dist/esm/pointer/keyMap.js
var defaultKeyMap = [
	{
		name: "MouseLeft",
		pointerType: "mouse",
...
//#endregion
//#region node_modules/@testing-library/user-event/dist/esm/system/pointer/buttons.js
...
var Buttons = class {
	getButtons() {
		let v = 0;
		for (const button of Object.keys(this.pressed)) v |= 2 ** Number(button);
		return v;
	}
	down(keyDef) {
...
function getMouseButtonId(button = 0) {
	if (button in MouseButton) return MouseButton[button];
...
function getMouseEventButton(button) {
	...
	if (button in MouseButtonFlip) return MouseButtonFlip[button];
	return button;
}
//#endregion
//#region node_modules/@testing-library/user-event/dist/esm/system/pointer/device.js
...
//#endregion
//#region node_modules/@testing-library/user-event/dist/esm/utils/misc/getTreeDiff.js
function getTreeDiff(a, b) {
	...
	let i = 0;
	for (;; i++) if (i >= treeA.length || i >= treeB.length || treeA[treeA.length - 1 - i] !== treeB[treeB.length - 1 - i]) break;
	return [
...
//#endregion
//#region node_modules/@testing-library/user-event/dist/esm/event/selection/resolveCaretPosition.js
function resolveCaretPosition({ target, node, offset }) {
	if (hasOwnSelection(target)) return {
		node: target,
...
function findNodeAtTextOffset(node, offset, isRoot = true) {
	...
	while (offset === void 0 ? i >= (isRoot ? Math.max(node.childNodes.length - 1, 0) : 0) : i <= node.childNodes.length) {
		...
		if (text.length) {
			if (offset !== void 0 && text.length < offset) offset -= text.length;
			else if (c.nodeType === 1) return findNodeAtTextOffset(c, offset, false);
...
//#endregion
//#region node_modules/@testing-library/user-event/dist/esm/event/selection/setSelectionPerMouse.js
function setSelectionPerMouseDown({ document, target, clickCount, node, offset }) {
	...
	if (targetHasOwnSelection) {
		...
		selection === null || selection === void 0 || selection.addRange(range.cloneRange());
		return range;
	}
}
function getTextRange(text, pos, clickCount) {
	...
	if (clickCount % 3 === 2) return [textPos - text.substr(0, pos).match(/(\w+|\s+|\W)?$/)[0].length, pos === void 0 ? pos : pos + text.substr(pos).match(/^(\w+|\s+|\W)?/)[0].length];
	return [textPos - text.substr(0, pos).match(/[^\r\n]*$/)[0].length, pos === void 0 ? pos : pos + text.substr(pos).match(/^[^\r\n]*/)[0].length];
}
//#endregion
//#region node_modules/@testing-library/user-event/dist/esm/event/selection/modifySelectionPerMouse.js
function modifySelectionPerMouseMove(selectionRange, { document, target, node, offset }) {
	...
	if ("node" in selectionRange) {
		...
		if (cmp < 0) range.setStart(selectionFocus.node, selectionFocus.offset);
		else if (cmp > 0) range.setEnd(selectionFocus.node, selectionFocus.offset);
		const selection = document.getSelection();
		selection === null || selection === void 0 || selection.removeAllRanges();
		selection === null || selection === void 0 || selection.addRange(range.cloneRange());
	}
...
//#endregion
//#region node_modules/@testing-library/user-event/dist/esm/system/pointer/shared.js
...
//#endregion
//#region node_modules/@testing-library/user-event/dist/esm/system/pointer/mouse.js
...
*/ var Mouse = class {
	move(instance, position, isPrevented) {
		...
		if (!isDifferentPointerPosition(prevPosition, position)) return;
	...
	down(instance, keyDef, isPrevented) {
		...
		if (!disabled && getMouseEventButton(keyDef.button) === 2) instance.dispatchUIEvent(target, "contextmenu", this.getEventInit("contextmenu", keyDef.button));
	}
		...
		if (!isDisabled(target)) {
			...
			if (clickTarget) {
				...
				if (init.detail) {
					...
					if (init.button === 0 && init.detail === 2) instance.dispatchUIEvent(clickTarget, "dblclick", {
	...
	getEventInit(type, button) {
		...
		if (type === "mousedown") init.detail = this.clickCount.getOnDown(init.button);
		else if (type === "mouseup") init.detail = this.clickCount.getOnUp(init.button);
		else if (type === "click" || type === "auxclick") init.detail = this.clickCount.incOnClick(init.button);
		return init;
	}
	getTarget(instance) {
	...
	constructor() {
		...
		_define_property$3(this, "clickCount", new class {
			incOnClick(button) {
				...
				this.count = this.count[button] === void 0 ? {} : { [button]: Number(this.count[button]) + 1 };
				return current;
			}
			getOnDown(button) {
				...
				this.count = { [button]: (_this_count_button1 = this.count[button]) !== null && _this_count_button1 !== void 0 ? _this_count_button1 : 0 };
...
//#endregion
//#region node_modules/@testing-library/user-event/dist/esm/utils/pointer/cssPointerEvents.js
...
function closestPointerEventsDeclaration(element) {
	...
	for (let el = element, tree = []; el === null || el === void 0 ? void 0 : el.ownerDocument; el = el.parentElement) {
		...
		if (pointerEvents && !["inherit", "unset"].includes(pointerEvents)) return {
			pointerEvents,
			tree
		};
	}
...
function checkPointerEvents(instance, element) {
	...
	if (!(instance.config.pointerEventsCheck !== PointerEventsCheckLevel.Never && (!lastCheck || hasBitFlag(instance.config.pointerEventsCheck, PointerEventsCheckLevel.EachApiCall) && lastCheck[ApiLevel.Call] !== getLevelRef(instance, ApiLevel.Call) || hasBitFlag(instance.config.pointerEventsCheck, PointerEventsCheckLevel.EachTrigger) && lastCheck[ApiLevel.Trigger] !== getLevelRef(instance, ApiLevel.Trigger)))) return lastCheck === null || lastCheck === void 0 ? void 0 : lastCheck.result;
...
function assertPointerEvents(instance, element) {
	...
	if ((declaration === null || declaration === void 0 ? void 0 : declaration.pointerEvents) === "none") throw new Error([
...
function getLabelDescr(element) {
	...
	if (element.hasAttribute("aria-label")) label = element.getAttribute("aria-label");
		...
		"select",
		"textarea"
	]) && ((_element_labels = element.labels) === null || _element_labels === void 0 ? void 0 : _element_labels.length)) label = Array.from(element.labels).map((el) => {
		var _el_textContent;
	...
	if (Number(label === null || label === void 0 ? void 0 : label.length) > 30) label = `${label === null || label === void 0 ? void 0 : label.substring(0, 29)}…`;
...
//#endregion
//#region node_modules/@testing-library/user-event/dist/esm/system/pointer/pointer.js
...
//#endregion
//#region node_modules/@testing-library/user-event/dist/esm/system/pointer/index.js
...
var PointerHost = class {
	...
	async press(instance, keyDef, position) {
		...
		if (pointer.pointerType !== "touch") this.mouse.position = position;
		if (pointer.pointerType === "touch") pointer.init(instance);
		pointer.down(instance, keyDef.button);
		if (pointer.pointerType !== "touch") this.mouse.down(instance, keyDef, pointer.isPrevented);
	}
	async move(instance, pointerName, position) {
		...
		const mousemove = pointer.pointerType === "touch" ? void 0 : this.mouse.move(instance, position, pointer.isPrevented);
		pointermove === null || pointermove === void 0 || pointermove.leave();
		mousemove === null || mousemove === void 0 || mousemove.leave();
		pointermove === null || pointermove === void 0 || pointermove.enter();
		mousemove === null || mousemove === void 0 || mousemove.enter();
		pointermove === null || pointermove === void 0 || pointermove.move();
		mousemove === null || mousemove === void 0 || mousemove.move();
	}
	async release(instance, keyDef, position) {
		...
		pointer.position = position;
		if (pointer.pointerType !== "touch") this.mouse.position = position;
		if (device.countPressed === 0) pointer.up(instance, keyDef.button);
		if (pointer.pointerType === "touch") pointer.release(instance);
	...
	constructor(system) {
		...
		_define_property$1(this, "pointers", new class {
			new(pointerName, pointerType, buttons) {
				...
				if (!isPrimary) Object.values(this.registry).forEach((p) => {
					if (p.pointerType === pointerType && !p.isCancelled) p.isMultitouch = true;
				});
...
//#endregion
//#region node_modules/@testing-library/user-event/dist/esm/system/index.js
...
//#endregion
//#region node_modules/@testing-library/user-event/dist/esm/convenience/click.js
async function click$1(element) {
	const pointerIn = [];
	if (!this.config.skipHover) pointerIn.push({ target: element });
	pointerIn.push({
		keys: "[MouseLeft]",
		target: element
	});
...
//#endregion
//#region node_modules/@testing-library/user-event/dist/esm/convenience/hover.js
...
//#endregion
//#region node_modules/@testing-library/user-event/dist/esm/convenience/tab.js
async function tab$1({ shift } = {}) {
	return this.keyboard(shift === true ? "{Shift>}{Tab}{/Shift}" : shift === false ? "[/ShiftLeft][/ShiftRight]{Tab}" : "{Tab}");
}
//#endregion
//#region node_modules/@testing-library/user-event/dist/esm/utils/keyDef/readNextDescriptor.js
...
*/ function readNextDescriptor(text, context) {
	...
	const type = new RegExp(`^\\${startBracket}{2}`).test(text) ? "" : startBracket;
	return {
		type,
...
function readTag(text, pos, startBracket, context) {
	...
	if (!endBracket) throw new Error(getErrorMessage([
...
//#endregion
//#region node_modules/@testing-library/user-event/dist/esm/keyboard/parseKeyDef.js
/**
...
//#endregion
//#region node_modules/@testing-library/user-event/dist/esm/keyboard/index.js
...
async function keyboardAction(instance, { keyDef, releasePrevious, releaseSelf, repeat }) {
	...
	if (system.keyboard.isKeyPressed(keyDef)) await system.keyboard.keyup(instance, keyDef);
	if (!releasePrevious) {
		for (let i = 1; i <= repeat; i++) {
			await system.keyboard.keydown(instance, keyDef);
			if (i < repeat) await wait(instance.config);
		}
		if (releaseSelf) await system.keyboard.keyup(instance, keyDef);
	}
}
async function releaseAllKeys(instance) {
	for (const k of instance.system.keyboard.getPressedKeys()) await instance.system.keyboard.keyup(instance, k);
}
//#endregion
//#region node_modules/@testing-library/user-event/dist/esm/document/copySelection.js
function copySelection(target) {
	...
	for (const type in data) if (data[type]) dt.setData(type, data[type]);
	return dt;
}
...
//#endregion
//#region node_modules/@testing-library/user-event/dist/esm/clipboard/copy.js
async function copy$1() {
	...
	if (this.dispatchUIEvent(target, "copy", { clipboardData }) && this.config.writeToClipboard) await writeDataTransferToClipboard(doc, clipboardData);
	return clipboardData;
}
//#endregion
//#region node_modules/@testing-library/user-event/dist/esm/clipboard/cut.js
async function cut$1() {
	...
	const clipboardData = copySelection(target);
	if (clipboardData.items.length === 0) return;
	if (this.dispatchUIEvent(target, "cut", { clipboardData }) && this.config.writeToClipboard) await writeDataTransferToClipboard(target.ownerDocument, clipboardData);
	return clipboardData;
}
//#endregion
//#region node_modules/@testing-library/user-event/dist/esm/clipboard/paste.js
...
//#endregion
//#region node_modules/@testing-library/user-event/dist/esm/pointer/parseKeyDef.js
...
//#endregion
//#region node_modules/@testing-library/user-event/dist/esm/pointer/index.js
async function pointer$1(input) {
	const { pointerMap } = this.config;
	const actions = [];
	(Array.isArray(input) ? input : [input]).forEach((actionInput) => {
		if (typeof actionInput === "string") actions.push(...parseKeyDef(pointerMap, actionInput));
		else if ("keys" in actionInput) actions.push(...parseKeyDef(pointerMap, actionInput.keys).map((i) => ({
			...actionInput,
			...i
		})));
		else actions.push(actionInput);
	});
	for (let i = 0; i < actions.length; i++) {
		await wait(this.config);
...
//#endregion
//#region node_modules/@testing-library/user-event/dist/esm/utility/clear.js
async function clear$1(element) {
	if (!isEditable(element) || isDisabled(element)) throw new Error("clear()` is only supported on editable elements.");
	focusElement(element);
	if (element.ownerDocument.activeElement !== element) throw new Error("The element to be cleared could not be focused.");
	selectAll(element);
	if (!isAllSelected(element)) throw new Error("The element content to be cleared could not be selected.");
	input(this, element, "", "deleteContentBackward");
}
//#endregion
//#region node_modules/@testing-library/user-event/dist/esm/utility/selectOptions.js
...
async function selectOptionsBase(newValue, select, values) {
	...
	const selectedOptions = valArray.map((val) => {
		if (typeof val !== "string" && allOptions.includes(val)) return val;
			...
			else throw getConfig().getElementError(`Value "${String(val)}" not found in options`, select);
		}
	...
	if (isElementType(select, "select")) if (select.multiple) for (const option of selectedOptions) {
		...
		selectOption(option);
		if (withPointerEvents) this.dispatchUIEvent(option, "click");
		await wait(this.config);
	}
	else if (selectedOptions.length === 1) {
		const withPointerEvents = this.config.pointerEventsCheck === 0 ? true : hasPointerEvents(this, select);
		if (withPointerEvents) await this.click(select);
		else focusElement(select);
	...
	else if (select.getAttribute("role") === "listbox") for (const option of selectedOptions) {
...
//#endregion
//#region node_modules/@testing-library/user-event/dist/esm/utility/type.js
async function type$1(element, text, { skipClick = this.config.skipClick, skipAutoClose = this.config.skipAutoClose, initialSelectionStart, initialSelectionEnd } = {}) {
	...
	if (!skipClick) await this.click(element);
	if (initialSelectionStart !== void 0) setSelectionRange(element, initialSelectionStart, initialSelectionEnd !== null && initialSelectionEnd !== void 0 ? initialSelectionEnd : initialSelectionStart);
	await this.keyboard(text);
	if (!skipAutoClose) await releaseAllKeys(this);
}
//#endregion
//#region node_modules/@testing-library/user-event/dist/esm/utils/edit/setFiles.js
		...
		value: {
			...
			set(v) {
				if (v === "") restore();
					...
					var _valueDescr_set;
					valueDescr === null || valueDescr === void 0 || (_valueDescr_set = valueDescr.set) == null || _valueDescr_set.call(el, v);
				}
			}
		},
		type: {
...
//#endregion
//#region node_modules/@testing-library/user-event/dist/esm/utility/upload.js
async function upload$1(element, fileOrFiles) {
	const input = isElementType(element, "label") ? element.control : element;
	if (!input || !isElementType(input, "input", { type: "file" })) throw new TypeError(`The ${input === element ? "given" : "associated"} ${input === null || input === void 0 ? void 0 : input.tagName} element does not accept file uploads`);
	if (isDisabled(element)) return;
	const files = (Array.isArray(fileOrFiles) ? fileOrFiles : [fileOrFiles]).filter((file) => !this.config.applyAccept || isAcceptableFile(file, input.accept)).slice(0, input.multiple ? void 0 : 1);
	const fileDialog = () => {
		var _input_files;
...
function isAcceptableFile(file, accept) {
	...
	return normalize(accept).trim().split(/\s*,\s*/).some((acceptToken) => {
		if (acceptToken.startsWith(".")) return normalize(file.name).endsWith(acceptToken);
		else if (wildcards.includes(acceptToken)) return normalize(file.type).startsWith(acceptToken.replace("*", ""));
		return normalize(file.type) === acceptToken;
	});
}
//#endregion
//#region node_modules/@testing-library/user-event/dist/esm/setup/api.js
var userEventApi = {
	click: click$1,
	dblClick: dblClick$1,
...
//#endregion
//#region node_modules/@testing-library/user-event/dist/esm/setup/wrapAsync.js
/**
...
//#endregion
//#region node_modules/@testing-library/user-event/dist/esm/setup/setup.js
/**
...
*/ function setupMain(options = {}) {
	...
	attachClipboardStubToView((_config_document_defaultView = config.document.defaultView) !== null && _config_document_defaultView !== void 0 ? _config_document_defaultView : 	/* istanbul ignore next */ globalThis.window);
...
function createInstance(config, system = new System()) {
	...
	return {
		instance,
		api: {
			...Object.fromEntries(Object.entries(userEventApi).map(([name, api]) => [name, wrapAndBindImpl(instance, api)])),
...
//#endregion
//#region node_modules/@testing-library/user-event/dist/esm/setup/directApi.js
var directApi_exports = /* @__PURE__ */ __exportAll({
	clear: () => clear,
	click: () => click,
	copy: () => copy,
	cut: () => cut,
...
//#endregion
//#region node_modules/@testing-library/user-event/dist/esm/setup/index.js
var userEvent = {
	...directApi_exports,
	setup: setupMain
};
//#endregion
export { PointerEventsCheckLevel, userEvent as default, userEvent };
